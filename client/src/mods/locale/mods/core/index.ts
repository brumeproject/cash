...
export const YouClaimedX: Localized<(x: string) => string> = {
  en: (x) => `You claimed ${x}`,
  zh: (x) => `您声称了 ${x}`,
  hi: (x) => `आपने ${x} दावा किया`,
  es: (x) => `Reclamaste ${x}`,
  ar: (x) => `ادعيت ${x}`,
  fr: (x) => `Vous avez revendiqué ${x}`,
  de: (x) => `Sie haben ${x} beansprucht`,
  ru: (x) => `Вы заявили ${x}`,
  pt: (x) => `Você reivindicou ${x}`,
  ja: (x) => `${x} を主張しました`,
  pa: (x) => `ਤੁਸੀਂ ${x} ਦਾਅਵਾ ਕੀਤਾ`,
  bn: (x) => `আপনি ${x} দাবি করেছেন`,
  id: (x) => `Anda mengklaim ${x}`,
  ur: (x) => `آپ نے ${x} دعویٰ کیا`,
  ms: (x) => `Anda telah menuntut ${x}`,
  it: (x) => `Hai reclamato ${x}`,
  tr: (x) => `${x} iddia ettiniz`,
  ta: (x) => `நீங்கள் ${x} கோர்ந்துகொண்டீர்கள்`,
  te: (x) => `మీరు ${x} దాదాపు చెప్పారు`,
  ko: (x) => `${x}를 주장했습니다`,
  vi: (x) => `Bạn đã tuyên bố ${x}`,
  pl: (x) => `Zażądałeś ${x}`,
  ro: (x) => `Ai revendicat ${x}`,
  nl: (x) => `U heeft ${x} geclaimd`,
  el: (x) => `Απαιτήσατε ${x}`,
  th: (x) => `คุณอ้าง ${x}`,
  cs: (x) => `Tvrdili jste ${x}`,
  hu: (x) => `Állítottál ${x}`,
